...
function UploadComponent({ userEmail, detailsComplete }) {
  ...
  const handleSaveResult = async () => {
     ...
     if (!result.filename) {
        setStatusMessage({ message: 'Error: Filename missing from analysis result. Cannot construct image URL.', type: 'error'});
        return;
     }

     setSaving(true);
     setStatusMessage({ message: 'Saving result...', type: 'info' });

     const imageUrlForDB = `/uploads/${result.filename}`;
    ...
    try {
      ...
      const data = await response.json();
        ...
        if (response.ok && data.success) {
            setStatusMessage({ message: data.message || "Result saved successfully!", type: 'success' });
        setImage(null);
        setPreview('');
        setResult(null);
        setShowResult(false);
