...
app.post("/api/upload", async (req, res) => {
    console.log("Received request to store image data:", req.body);
    try {
        const { 
            imageName, 
            imageUrl, 
        ...
        // Log the imageUrl being stored
        console.log('Image URL being stored:', imageUrl);

        // Basic validation of received data
        if (!imageName || !imageUrl || !classification || typeof confidence === 'undefined' || !userEmail) {
            console.log("❌ Missing required fields in request body for storing image.");
            return res.status(400).json({ success: false, error: "Missing required image data fields." });
        }
       
        // Create and Save RoadImage document
        const newImage = new RoadImage({
            imageName: imageName,
            imageUrl: `/uploads/${imageName}`, // Ensure consistent URL format
        ...
        await newImage.save();
        console.log(`✅ Image record saved via frontend data for user ${userEmail}: ${newImage._id}`);
        console.log('Saved image URL:', newImage.imageUrl);
        ...
        res.json({ 
            ...
            imageUrl: newImage.imageUrl // Return the URL for client-side use
        });

    } catch (error) {
        console.error("❌ Error saving image data to DB:", error.message);
        res.status(500).json({ success: false, error: error.message || "Server error while saving image data" });
    }
});
